...
export function handleQuery(q, ctx = {}) {
  ...
  if (/^(شكرا|شكرًا|يسلمو|يسلموا|مشكور|تسلم)\s*$/i.test(raw)) {
    return {
      ok: true,
      found: true,
      reply: "يسعدني 😊 إذا بدك أي شي بالتوصيل أو التبديل أو اقتراح منتج احكيلي شو بتفضل.",
      tags: ["thanks"]
    };
  }
  ...
  if (/بدي حدا احكي معاه|بدي احكي مع موظف|موظف|خدمة العملاء|بدي دعم/i.test(raw)) {
    return {
      ok: true,
      found: false,
      reply: "أكيد 🙏 رح أحوّل طلبك لموظف خدمة العملاء. اترك رقمك/اسمك لو سمحت وبيرجعولك بأقرب وقت.",
      tags: ["تصعيد"]
    };
  }
